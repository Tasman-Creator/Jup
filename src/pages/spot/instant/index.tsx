...
const Instant: React.FC<InstantProps> = ({ isAsideOpen }) => {
  ...
  /** USD value from last quote: inUsdValue (selling), outUsdValue (buying) */
  ...
  const handleSellingChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // buyingValue is set from quote in useEffect
  }
  ...
  // Quote for current selling amount: inUsdValue/outUsdValue → secondary-money, outAmount → buyingValue (human = amount/10^decimals)
  useEffect(() => {
    ...
    <>
      <div className="instant-container">
        ...
        {isConnected && !sellingValue || sellingValue === '0.00' ? (
          ...
          /*@ts-ignore*/
          <Flow sendingCurrencyValue={+sellingValue} sendingCurrencyPrice={+sellingPrice} text={'Swap'}/>
        )}
      </div>
          ...
          {tokens.map((token: Token) => {
            return (
              <CoinsItem
                ...
                active={(token.organicScore ?? 0) < 80}
